...
def get_invoices(filters):
    ...
    if party_type == "Supplier":
        ...
        prev_invoices = frappe.qb.from_(inv_form).select(
            inv_form.name.as_("invoice_id"), inv_form.has_supplier_commission_invoice,
            ((inv_form.grand_total * supplier_commission) / 100).as_("total")).where(
            inv_form.supplier == party).where(inv_form.has_supplier_commission_invoice == 0).where(
            inv_form.docstatus == 1).where(inv_form.posting_date.lt(from_date)).run(as_dict=True)
        if prev_invoices:
            return {
                "invoices": [],
                "success": False,
        ...
        invoices = frappe.qb.from_(inv_form).select(
            inv_form.name.as_("invoice_id"), inv_form.has_supplier_commission_invoice,
            ((inv_form.grand_total * supplier_commission) / 100).as_("total")).where(
            inv_form.supplier == party).where(inv_form.has_supplier_commission_invoice == 0).where(
        ...
        prev_invoices = frappe.qb.from_(inv_form).join(inv_frmitem).on(inv_frmitem.parent == inv_form.name).select(
            ...
            inv_frmitem.parent).run(as_dict=True)
        if prev_invoices:
            return {
                "invoices": [],
                "success": False,
